 ...
 export function mapBillToApi(bill: BillUI) {
  const peopleToAdd = bill.people
    ...
    .map(p => ({
      ...
      individualTipPercentage:                              // then that is what should be used, and tipAmount should not be added.
        p.individualTipPercentage  === bill.tipPercent ? undefined : p.individualAmount,
    }));
